...
if (!API_KEY) {
  console.warn("RESEND_API_KEY is not configured. Application confirmation emails will fail.")
}
...
export async function sendApplicationConfirmationEmail({ to, name, jobTitle }: EmailData) {
  if (!API_KEY) {
    throw new Error("Missing RESEND_API_KEY environment variable. Please set RESEND_API_KEY in your environment.")
  }

  // Build deadline and HTML outside retry loop
  ...
  console.error("All attempts to send application confirmation email failed:", lastError)
  throw new Error(`Failed to send application confirmation email after ${maxAttempts} attempts: ${lastError?.message || String(lastError)}`)
}
